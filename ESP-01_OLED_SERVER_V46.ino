...
struct data {
  int PULSE1, HCHP_tic1, HCHP_tic2, HCHP_tic3;
};

data ref_index = {0, 0, 0, 0};     
data current_index = {0, 0, 0, 0}; 

int HP_tic2_previous=0;
int HP_tic3_previous=0;
...
String SendHTML(int16_t ACTIVE_POWER,int16_t L1,int16_t L2,int16_t L3,int p_routed,int ST_HP,String routing){
  ...
  ptr +="</div>";
  ptr +="<div class='side-by-side text'>ST_HP</div>";
  ptr +="<div class='side-by-side reading'>";
  ptr +=(int)ST_HP;
  ptr +="<span class='superscript'>Wh</span></div>";
  ptr +="</div>";
...
void handle_OnConnect() {
  server.send(200, "text/html", SendHTML(ACTIVE_POWER,L1,L2,L3,p_routed,ST_HP,routing)); 
}
...
data esp_cgx() // fonction GET index compteur Cumulus
{
  ...
  { 
    ...
    return {PULSE1, HCHP_tic1, HCHP_tic2, HCHP_tic3}; 
...
void setup()
 {
  ...
  ESP.wdtEnable(30000);                                     // Watchdog enable   
...
void loop() {
  ...
  {                             
    ...
    // false if on-peak tariff (HP) is active
      ...
      if ((TARIFF == false) && (latch == false))  // on-peak tariff (HP) is active
        { 
        oled_index_display();
        ...
        latch=true;                                           // off-peak tariff (HC) is now active

        oled_index_display();
        current_index=esp_cgx();
        HP_tic3 = current_index.HCHP_tic3;                    // get current HCHP_tic3 of the day in HP tariff
        delta_HP_tic3 = (HP_tic3 - HP_tic3_previous);         // consumption TIC3 in HP tariff
        HP_tic3_previous = HP_tic3;                           // get previous HCHP_tic3 in HP tariff

        HP_tic2 = current_index.HCHP_tic2;                    // get current HCHP_tic2 of the day in HP tariff
        delta_HP_tic2 = (HP_tic2 - HP_tic2_previous);         // consumption TIC2 in HP tariff
        HP_tic2_previous = HP_tic2;                           // get previous HCHP_tic2 in HP tariff
  
        ST_HP = (delta_HP_tic2 - delta_HP_tic3);              // difference between TIC2 and TIC3 during HP tariff
      } 
